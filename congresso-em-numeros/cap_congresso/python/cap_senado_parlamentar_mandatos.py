...
class Parlamentar_Mandatos_Senado(Capture):
    ...
    def fill_entry_parlamentar_mandatos(self, exercicio, entry_columns):
...
if __name__ == '__main__':
    ...
    for (id_parlamentar,id_senado) in tqdm(list_of_parlamentares):
        ...
        cap = Parlamentar_Mandatos_Senado(table='parlamentar_mandatos', schema='c_congresso', entry_columns=entry_columns)
        cap.entry_columns['id_parlamentar'] = id_parlamentar

        url = 'http://legis.senado.leg.br/dadosabertos/senador/{}/mandatos'.format(id_senado)
        cap.entry_columns['url_captura'] = url
        data_source = utils.get_json(url)

        iterable = data_source['MandatoParlamentar']['Parlamentar']['Mandatos']['Mandato']
